...
void cmd_ls() {
    ...
    if (d) {
        ...
        printf("  Error: Cannot open zenith_hdd/\n");
    }
}

void cmd_cat(const char* filename) {
    char path[128];
    sprintf(path, "zenith_hdd/%s", filename);
    FILE* f = fopen(path, "r");
    if (f) {
        char line[256];
...
int main() {
    ...
    while (1) {
        ...
        else if (strcmp(input, "pwd") == 0) printf("  /zenith_hdd\n");
        else if (strncmp(input, "cat ", 4) == 0) cmd_cat(input + 4);
        else if (strncmp(input, "mkdir ", 6) == 0) {
            char path[128];
            sprintf(path, "zenith_hdd/%s", input + 6);
