...
void print_boot_line(const char* message) {
    ...
    usleep(400000); // Boot delay simulating Windows
}
...
void display_boot_screen(int ram_mb, int hdd_mb, int cores) {
    ...
    printf("\033[1;33mStarting Windows-like Boot Sequence...\033[0m\n\n");
    usleep(800000);

    print_boot_line("Initializing kernel...");
    printf("  \033[1;32m[ OK ]\033[0m RAM: %d MB\n", ram_mb);
    usleep(500000);
    printf("  \033[1;32m[ OK ]\033[0m HDD: %d MB\n", hdd_mb);
    usleep(500000);
    printf("  \033[1;32m[ OK ]\033[0m CPU Cores: %d\n", cores);
    usleep(500000);
    print_boot_line("Starting Scheduler...");
    print_boot_line("Starting IPC Listener...");
    printf("\n\033[1;36mBoot sequence complete. Starting Desktop...\033[0m\n");
    usleep(1200000);
}
...
void display_menu() {
    ...
    printf("  \033[1;33mZENITH OS V3\033[0m  |  RAM: %d/%d MB  |  Tasks: %d (%d active)\n",
           (total_RAM - free_RAM), total_RAM, bg_task_count, active_count);
    printf("\033[1;36m------------------------------------------------------------\033[0m\n");

    for (int i = 1; i <= 12; ++i) {
        printf("  \033[1;32m[%2d]\033[0m %-18s", i, task_list[i].display_name);
        if (i + 12 <= 23) printf(" \033[1;32m[%2d]\033[0m %s", i + 12, task_list[i + 12].display_name);
        printf("\n");
    }

    printf("\033[1;36m------------------------------------------------------------\033[0m\n");
    printf("  \033[1;35m[M]\033[0m Tasks  \033[1;35m[F]\033[0m Foreground  \033[1;35m[K]\033[0m Kernel  \033[1;35m[L]\033[0m Logs  \033[1;31m[0]\033[0m Shutdown\n");
    printf("  \033[1;32mZenithOS>\033[0m ");
    fflush(stdout);
}
...
int main() {
    init_logger();
    printf("  \033[1;36mZENITH OS CONFIGURATION\033[0m\n");
